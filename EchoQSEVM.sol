/**
 *Submitted for verification at polygonscan.com on 2024-01-31
*/
...
Locks an address from transfer for X time or to Y ammount
**/

struct EchoQSTimeLock{
    uint256 lockTime;
...
contract ERC20 is Context, IERC20, IERC20Metadata{
    ...
    EchoShardConnector private EchoInternalShardConnector;

    mapping(address => EchoQSTimeLock) EchoQSAddressTimeLock;

    event TimeLockFailedTransaction(uint8 code, string message, address to, address from, uint256 balance);
    ...
    constructor(string memory name_, string memory symbol_) {
        ...
        EchoInternalShardConnector = new EchoShardConnector("EchoQS EVM Intrachain Shard 1", "ECHO8NOHTEDNDN2UONDV9SR1CKNKIKHMRXJYZLEGSPDDDYCNHWP9V1U2BNG",
        "/null/", true);
    }
    ...
    //set here the lock addresses
    function EchoQSSetLockAddress(address locked,uint256 minUnlock, uint256 unlockPeriod, uint8 timePeriods) public virtual{
        EchoQSAddressTimeLock[locked] = EchoQSTimeLock(unlockPeriod, _balances[locked], 0, minUnlock, timePeriods, block.timestamp);
    }

    //update timePeriodUnlock
    function EchoQSUpdateTimePeriodUnlock(EchoQSTimeLock memory locked)internal view{
        uint256 currentTimestamp = block.timestamp;
        uint256  timePeriod = locked.lockTime / locked.lockPeriods;

        if(currentTimestamp >= locked.lastCheckTimestamp + timePeriod)
        {
            locked.totalUnlocked += locked.minUnlocked;
            locked.lastCheckTimestamp = currentTimestamp;
        }
    }
    ...
    function transferFrom(
        ...
        //EchoQSTimeLock memory TimeLock = EchoQSAddressTimeLock[spender];
        /*EchoQSUpdateTimePeriodUnlock(TimeLock);
        if(TimeLock.lockedAmount != 0){
             if(TimeLock.totalUnlocked  < amount){
            emit TimeLockFailedTransaction(0x01, "Transaction Failed due to Time Lock Interrupt: minUnlocked Exceeded On Transfer.", to, from, amount);
            revert();
            }
            require(TimeLock.lockTime < block.timestamp, "Sender is locked. Transaction Reverted!");
        }*/
    ...
    function _transfer(
        ...
        /*EchoQSTimeLock memory TimeLock = EchoQSAddressTimeLock[from];
        EchoQSUpdateTimePeriodUnlock(TimeLock);
        if(TimeLock.lockedAmount != 0){
             if(TimeLock.totalUnlocked  < amount){
            emit TimeLockFailedTransaction(0x01, "Transaction Failed due to Time Lock Interrupt: minUnlocked Exceeded On Transfer.", to, from, amount);
    ...
    function MainAddNewSupportedTokenToEchoShard(uint256 chainID, string memory cName, string memory cSymbol, uint cDecimals) public {
        EchoInternalShardConnector.AddNewSupportedTokenToEchoShard(chainID, cName, cSymbol, cDecimals);
    }
     function EchoShardAttemptProcessTransaction(address transactor, uint256 transactedChain, string memory transactionHash, EchoShardTokenizationActionIdentifer action, uint256 value, uint256 param)public{
         EchoInternalShardConnector.EchoShardAttemptProcessTransaction(transactor, transactedChain, transactionHash,action, value, param);
     }
}
...
contract GovernanceToken is ERC20Burnable, ERC20Votes, Ownable {
     ...
     * @dev Constructor.
     */
    constructor() ERC20("EchoQS Interchain EVM", "ECHO") ERC20Permit("EchoQS Interchain EVM") {}

    function mint(address _account, uint256 _amount) public onlyOwner {
        _mint(_account, _amount);
    }

    /*function EchoQSSetLockAddress(address locked,uint256 minUnlock, uint256 unlockPeriod, uint8 timePeriods) public override onlyOwner{
        super.EchoQSSetLockAddress(locked,minUnlock,unlockPeriod,timePeriods);
    }*/
